<?php
  ...
  if(isset($_POST['delete'])){
    ...
    if(($con->query($sql2) == true) && ($con->query($sql3) == true) && ($con->query($sql1) == true)) {
      header("location: adventures.php");
    } else {
        ...
        <table style="border:5px solid #fabe28; width:100%;">
          ...
          <tr style="color:white;">
            ...
            if($_SESSION["full_name"] == $rows['owner_name'] && $_SESSION["email_id"] == $rows['owner_email']) { ?>
              <form action="editAdventures.php?id=<?php echo $rows['post_id']?>" method="post"><a href="editAdventures.php?id=<?php echo $rows['post_id']?>"><input type="submit" value="Edit" name="edit" class="btn" id="<?php echo $rows['post_id'];?>" style="background-color:#33a396; font-weight:500; font-size: 1.2rem; 
              color: #fff; display: inline-block; padding: 2px 18px; margin-top: 10px; text-transform: uppercase; letter-spacing: 2px; text-decoration: none; width: 190px;"></a></form>

              <form action="adventures.php?id=<?php echo $rows['post_id']?>" method="post"><a href="adventures.php?id=<?php echo $rows['post_id']?>"><input type="submit" id="<?php echo $rows['post_id'];?>" value="Delete" name="delete" class="btn" style="background-color:#f51637; font-weight:500; font-size: 1.2rem; 
